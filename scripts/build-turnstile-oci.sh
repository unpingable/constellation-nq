...
IMAGE_REPOSITORY="turnstile.local/nq"
...
archive="${OUTPUT_ROOT}/turnstile-nq-${SOURCE_COMMIT}.oci.tar"
...
jq -cn \
    --arg schema nq.turnstile_image_identity.v1 \
    --arg campaign TURNSTILE \
    --arg slug "${SLUG}" \
    ...
    >"${rootfs}/etc/nq/turnstile-image.json"
chmod 0444 "${rootfs}/etc/nq/turnstile-image.json"
configuration_digest="sha256:$(sha256sum "${rootfs}/etc/nq/turnstile-image.json" | awk '{print $1}')"
...
jq -cn \
    ...
    '{architecture:"amd64",os:"linux",created:$created,config:{User:"65532:65532",Env:["PATH=/usr/bin:/usr/lib/nq/helpers"],Entrypoint:["/usr/bin/nq"],Cmd:["--version"],WorkingDir:"/",Labels:{"org.opencontainers.image.revision":$revision,"org.opencontainers.image.source":"TURNSTILE/k3s-ag-exact-occurrence-adapter-v1","nq.turnstile.nq-digest":$nq_digest,"nq.turnstile.passive-helper-digest":$helper_digest}},rootfs:{type:"layers",diff_ids:[$diff_id]},history:[{created:$created,created_by:"scripts/build-turnstile-oci.sh",comment:"exact TURNSTILE nq and passive helper"}]}' \
    >"${config_json}"
...
jq -cn \
    --arg schema nq.turnstile_oci_artifact_facts.v1 \
