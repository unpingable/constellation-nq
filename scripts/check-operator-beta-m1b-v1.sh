...
rg -F --quiet 'test_restart_waits_for_changed_boot_identity_not_sampled_downtime' "$tests"
...
rg -F --quiet 'test_post_effect_accepts_disabled_status_and_validates_exact_state' "$tests"
rg -F --quiet 'test_teardown_checks_protected_store_absence_as_owner' "$tests"
rg -F --quiet 'test_package_payload_absence_distinguishes_dpkg_record_states' "$tests"
rg -F --quiet 'test_package_payload_absence_matches_release_inventory' "$tests"
[[ $(rg -F -c 'sudo test ! -e /var/lib/nq/operator-beta.sqlite' "$runner") -eq 2 ]]
rg -F --quiet '"0:deinstall ok config-files"|"1:"' "$runner"
rg -F --quiet 'PACKAGE_PAYLOAD_ABSENCE_SCRIPT' "$runner"
rg -F --quiet '/usr/lib/nq/helpers/nq_conformance_helper.py' "$runner"
! rg -F --quiet '! dpkg-query -W nq-ng' "$runner"
...
rg -F --quiet 'self.wait_boot_identity_change(guest, before_by_role[guest.role], 900)' "$runner"
