...
BASE_IMAGE="${BEDROCK_BASE_IMAGE:-/data/git/skunkworks/nq-ng/.campaign-local/track-b-vm-deployment-675e247/base.qcow2}"
...
USER_NAME="bedrock"
CLUSTER_NETWORK="10.90.0.0/24"
CLUSTER_MCAST="230.90.0.1:19002,localaddr=127.0.0.1"
...
node_ip() {
    case "$1" in
        server) printf '10.90.0.11\n' ;;
        agent) printf '10.90.0.12\n' ;;
        *) die "unknown node: $1" ;;
    esac
}
...
node_mac_nat() {
    case "$1" in
        server) printf '52:54:00:90:00:11\n' ;;
        agent) printf '52:54:00:90:00:12\n' ;;
        *) die "unknown node: $1" ;;
    esac
}
...
node_mac_cluster() {
    case "$1" in
        server) printf '52:54:00:90:01:11\n' ;;
        agent) printf '52:54:00:90:01:12\n' ;;
        *) die "unknown node: $1" ;;
    esac
}
...
prepare_node() {
    ...
    printf '%s\n' \
        ...
        "local-hostname: bedrock-k3s-${node}" \
        >"${directory}/seed/meta-data"

    cat >"${directory}/seed/user-data" <<EOF
#cloud-config
hostname: bedrock-k3s-${node}
...
runcmd:
  - [swapoff, -a]
  - [mkdir, -p, /mnt/bedrock-custody]
  - [sh, -c, "mount -t 9p -o trans=virtio,version=9p2000.L bedrock_custody /mnt/bedrock-custody"]
  - [sh, -c, "grep -q '^bedrock_custody ' /etc/fstab || printf '%s\\n' 'bedrock_custody /mnt/bedrock-custody 9p trans=virtio,version=9p2000.L,_netdev 0 0' >> /etc/fstab"]
EOF
...
start_node() {
    ...
    qemu-system-x86_64 \
        ...
        -virtfs "local,path=${LAB_ROOT}/external-custody,mount_tag=bedrock_custody,security_model=mapped-xattr,id=bedrock_custody0,multidevs=remap" \
        -display none \
...
teardown_node() {
    ...
    pid="$(<"${directory}/qemu.pid")"
    if ! kill -0 "${pid}" 2>/dev/null; then
        printf 'BEDROCK refusal: stale pid file retained for review: %s\n' "${directory}/qemu.pid" >&2
        return 1
    fi
