...
"""
    ...
    def install_inputs(self) -> tuple[Guest, Guest]:
        ...
        self.scp_to(target, [nq_deb, ag_deb], "/home/betaoperator/")
        self.ssh(
            control,
            "sha256sum /home/betaoperator/nq-ng.deb; "
...
"""
    ...
    def execute_diagnostic(
        ...
        if artifact.get("profile", {}).get("id") != expected_profile:
            raise Refusal(f"{instance} emitted the wrong profile identity")
        if artifact.get("threshold_policy", {}).get("version") != self.run_id:
    ...
    def enact(self, target: Guest, bindings: dict[str, Any]) -> dict[str, Any]:
        ...
        if outcome.get("outcome") != "success":
            raise Refusal("fresh AG M1A occurrence did not return exact success")
        record = {
    ...
    def package_continuity(
        ...
        for guest in (control, target):
            result = self.ssh(
                ...
                "test ! -s /var/lib/nq/operator-beta.sqlite-wal; "
                "before=$(sudo sha256sum /var/lib/nq/operator-beta.sqlite | cut -d ' ' -f1); "
                "sudo dpkg -r nq-ng; "
                "test -f /var/lib/nq/operator-beta.sqlite; "
            ...
            if self.created:
                refusal = {
                    ...
                    "effect_outcome": (
                        ...
                        if self.last_completed == "pre_effect_recorded"
                        else "NO_NEW_CLAIM"
                    ),
                }
