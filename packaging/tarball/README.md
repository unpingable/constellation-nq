...
`nq-linode-origin-helper`, and `nq-passive-load-helper` binaries plus a profile
catalog verified against the supplied `nq`. The
...
staging. Release automation must still supply the five binaries as one
