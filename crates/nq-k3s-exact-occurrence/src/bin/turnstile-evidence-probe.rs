//! Authority-neutral T9 probe for the TURNSTILE external journal backend.
...
const FIXTURE_ID: &str = "TURNSTILE-T9-AUTHORITY-NEUTRAL-CUSTODY-PROBE-V1";
...
fn binding() -> EvidenceBindingV1 {
    EvidenceBindingV1 {
        canonical_custody_id: sha256_bytes(b"TURNSTILE T9 canonical custody probe"),
        selection_contract_digest: sha256_bytes(b"TURNSTILE T9 selection contract probe"),
        external_journal_id: sha256_bytes(b"TURNSTILE T9 external journal probe"),
        receipt_destination_id: sha256_bytes(b"TURNSTILE T9 receipt destination probe"),
...
fn custody() -> ExternalEvidenceCustodyV1 {
    ...
    ExternalEvidenceCustodyV1 {
        ...
        writer_domain: sha256_bytes(b"TURNSTILE T9 exclusive writer domain probe"),
    }
}
...
fn genesis() -> EvidenceEventBodyV1 {
    EvidenceEventBodyV1 {
        ...
        predecessor: None,
        journal_id: binding().external_journal_id,
        plan_id: sha256_bytes(b"TURNSTILE T9 non-authorizing fixture plan"),
        nq_occurrence: sha256_bytes(b"TURNSTILE T9 non-authorizing fixture occurrence"),
        docket: None,
        runtime: None,
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result = ProbeResult {
        schema: "nq.turnstile_filesystem_evidence_probe_result.v1",
...
fn main() -> ExitCode {
    match run() {
        Ok(()) => ExitCode::SUCCESS,
        Err(error) => {
            eprintln!("TURNSTILE evidence probe refused: {error}");
            ExitCode::FAILURE
        }
    }
}
