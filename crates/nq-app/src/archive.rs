...
mod tests {
    ...
    fn migrated_v3_archive(root: &Path) -> PathBuf {
        ...
        let backup_path = root.join("nq-v5.pre-upgrade.db");
        let backup = Store::backup_v5_verified(&database, &backup_path).expect("verify v5 backup");
        let receipt = nq_store::UpgradeReceiptInput {
            receipt_id: "upgrade-archive-v5-v12".into(),
            ...
            backup_digest: backup.sha256,
            backup_location: backup.path.to_string_lossy().into_owned(),
            started_at: "2026-07-20T12:00:06Z".into(),
            finished_at: "2026-07-20T12:00:07Z".into(),
            result: "migrated".into(),
            operator_identity: canonical(&serde_json::json!({"uid":991})),
            verification: canonical(
                &serde_json::json!({"integrity":"ok", "source_schema_version":5,
                "source_schema_artifact_digest":nq_store::SCHEMA_V5_ARTIFACT_DIGEST,
                "backup_reopened":true,"historical_saved_checks":"absent_not_synthesized",
